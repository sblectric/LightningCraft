...
public class ChunkProviderUnderworld implements IChunkGenerator {
	...
	/** Get the rough terrain of the Underworld here */
	...
	/** Do some block replacement */
	public void replaceBiomeBlocks(int par1, int par2, IBlockState[] blocks, Biome[] biomes) {

		short stoneGrassCanGen = chunkHeight - 64;
		short sandCanGen = chunkHeight / 8 - 1;
		double d0 = 0.03125D;
		this.grassNoise = this.grassandstoneNoiseGen.generateNoiseOctaves(this.grassNoise, par1 * 16, par2 * 16, 0, 16, 16, 1, d0, d0, 1.0D);
		this.stoneNoise = this.grassandstoneNoiseGen.generateNoiseOctaves(this.stoneNoise, par1 * 16, 109, par2 * 16, 16, 1, 16, d0, 1.0D, d0);
		this.understoneExclusivityNoise = this.understoneExculsivityNoiseGen.generateNoiseOctaves(
				this.understoneExclusivityNoise, par1 * 16, par2 * 16, 0, 16, 16, 1, d0 * 2.0D, d0 * 2.0D, d0 * 2.0D);

		// go block by block through the chunks
		for (int k = 0; k < 16; ++k)
		{
			for (int l = 0; l < 16; ++l)
			{
				...
				{
					int l1 = (l * 16 + k) * chunkHeight + y;

					if (y < (chunkHeight - 1) - this.dimRNG.nextInt(5) && y > 0 + this.dimRNG.nextInt(5))
					{
						...
						if (block2 != null && block2.getBlock().getMaterial(block) != Material.AIR)
						{
							if (block2 == LCBlocks.stoneBlock.getStateFromMeta(BlockStone.UNDER))
							{
								if (j1 == -1)
								{
									...
									// replace the regular understone with stone or dirt/grass blocks
									...
									// replace dirt with undersand at low levels
									if(block1 == Blocks.DIRT.getDefaultState() && y <= sandCanGen) {
										block1 = LCBlocks.underSand.getDefaultState();
									}

									// generate ice patches
									if (y < stoneGrassCanGen && (block == null || block.getBlock().getMaterial(block) == Material.AIR))
									{
										// but gen sand instead at low levels
										if(y <= sandCanGen) {
											block = LCBlocks.underSand.getDefaultState();
										} else {
									...
									{
										if(y + 1 < chunkHeight && (blocks[l1 + 1] == null || blocks[l1 + 1].getBlock().getMaterial(block) == Material.AIR)) {
											blocks[l1] = block;
										} else {
											blocks[l1] = block1;
										}
	...
	{
		boolean canAccessWorld = this.genUnderworldTower.canAccessWorld() && 
	...
	public void recreateStructures(Chunk chunkIn, int x, int z) {
		this.genUnderworldWaterTemple.generate(this.worldObj, x, z, null);
		this.genUnderworldTower.generate(this.worldObj, x, z, null);
		this.genUnderworldRampart.generate(this.worldObj, x, z, null);
	}
