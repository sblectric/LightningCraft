...
public class BlockLightningFurnace extends BlockContainerLC implements IFurnace {
	...
	public BlockLightningFurnace() {
		super(Blocks.FURNACE, 3.0f, 20.0f);
	...
	public int getMetaFromState(IBlockState state) {
	    EnumFacing type = (EnumFacing)state.getValue(dir);
	...
	public void randomDisplayTick(IBlockState state, World world, BlockPos pos, Random rand) {
		if(isBurning(state, world, pos)) {
			EnumFacing enumfacing = (EnumFacing)state.getValue(dir);
			...
			case EAST:
				...
				break;
			case NORTH:
				world.spawnParticle(EnumParticleTypes.REDSTONE, d0 + d4, d1, d2 - d3, 0.0D, 0.0D, 0.0D);
				break;
			case SOUTH:
				world.spawnParticle(EnumParticleTypes.REDSTONE, d0 + d4, d1, d2 + d3, 0.0D, 0.0D, 0.0D);
				break;
