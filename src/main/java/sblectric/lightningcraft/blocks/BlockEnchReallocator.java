...
public class BlockEnchReallocator extends BlockContainerLC implements IFurnace {
	...
	public int getLightValue(IBlockState state, IBlockAccess world, BlockPos pos) {
		return isBurning(world.getBlockState(pos), world, pos) ? 4 : 0;
	}
	...
	public void randomDisplayTick(IBlockState state, World world, BlockPos pos, Random rand) {
		if(isBurning(state, world, pos)) {
			...
			world.spawnParticle(EnumParticleTypes.PORTAL, d0 - d3, d1, d2 + d4, 0.0D, 0.0D, 0.0D);
			world.spawnParticle(EnumParticleTypes.PORTAL, d0 + d3, d1, d2 + d4, 0.0D, 0.0D, 0.0D);
			world.spawnParticle(EnumParticleTypes.PORTAL, d0 + d4, d1, d2 - d3, 0.0D, 0.0D, 0.0D);
			world.spawnParticle(EnumParticleTypes.PORTAL, d0 + d4, d1, d2 + d3, 0.0D, 0.0D, 0.0D);
		}
	}
